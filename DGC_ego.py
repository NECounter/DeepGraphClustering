...
adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, allY = process_ego.load_data(datasetName)
dataDistribute = [train_mask.argmin(), 110, 181]
...
featuresCopy = copy.deepcopy(features).tolist()
...
if doCluster:
    for i in range(init_seekIters):
        ...
        if evl[0] > NMI_pre:
            ...
            NMI_pre = evl[0]
    np.save('records/init_pred_'+datasetName,init_pred)
else:
    init_pred = np.load('records/init_pred_'+datasetName+'.npy').tolist()
